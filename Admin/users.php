        ...
        switch ($_POST['action']) {
            case 'add_user':
                ...
                if ($role === 'Customer') {
                    ...
                    // Insert into users table - set created_by and updated_by to NULL since admin is not in users table
                    $stmt = $pdo->prepare("
                    ...
                    // Sub Admin
                    // Check if email exists in sub_admins table
                    $stmt = $pdo->prepare("SELECT sub_admin_id FROM sub_admins WHERE email = ?");
                    $stmt->execute([$email]);
                    if ($stmt->rowCount() > 0) {
                        echo json_encode(['success' => false, 'message' => 'Email already exists in sub admins']);
                        break;
                    }

                    // Insert into sub_admins table
                    $stmt = $pdo->prepare("
                        INSERT INTO sub_admins (full_name, email, password, status, created_at, updated_at, last_password_change) 
                        VALUES (?, ?, ?, ?, NOW(), NOW(), NOW())
                    ");
                    $result = $stmt->execute([$name, $email, $hashedPassword, $status]);
                    ...
                    if ($result) {
                        ...
                        echo json_encode(['success' => false, 'message' => 'Failed to add sub admin']);
                    }
                }
                break;

            case 'toggle_user_status':
                $type = $_POST['type']; // 'customer' or 'staff'
                $id = $_POST['id'];
                $currentStatus = $_POST['current_status'];
                $newStatus = ($currentStatus === 'active') ? 'inactive' : 'active';

                if ($type === 'customer') {
                    // Update users table - set updated_by to NULL since admin is not in users table
                    $stmt = $pdo->prepare("
                        UPDATE users 
                    ...
                    if ($result) {
                        echo json_encode(['success' => true, 'message' => 'Sub Admin status updated', 'new_status' => $newStatus]);
                    } else {
            ...
            case 'update_user':
                ...
                if ($type === 'customer') {
                    ...
                    if (!empty($newPassword)) {
                        ...
                        $stmt = $pdo->prepare("
                            UPDATE sub_admins 
                            SET full_name = ?, email = ?, password = ?, status = ?, updated_at = NOW(), last_password_change = NOW()
                            WHERE sub_admin_id = ?
                        ");
                        $result = $stmt->execute([$name, $email, $hashedPassword, $status, $id]);
                    } else {
                        $stmt = $pdo->prepare("
                            UPDATE sub_admins 
                            SET full_name = ?, email = ?, status = ?, updated_at = NOW()
                            WHERE sub_admin_id = ?
                        ");
                        $result = $stmt->execute([$name, $email, $status, $id]);
                    }
...
// Get all sub admins - FIXED: Ensure status has default value
$subAdmins = [];
try {
    $stmt = $pdo->query("
        SELECT 
...
// Get notifications count (recent registrations)
...
try {
    ...
    // Get recent sub admin additions (last 24 hours)
    $stmt = $pdo->query("
        SELECT COUNT(*) as count 
        FROM sub_admins 
        WHERE created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
    ");
    $subAdminAdditions = $stmt->fetch()['count'];

    $notification_count = $userRegistrations + $subAdminAdditions;
    ...
    if ($subAdminAdditions > 0) {
        $notifications[] = [
            'icon' => 'shield',
            'title' => 'New sub admin additions',
            'meta' => "$subAdminAdditions new sub admin(s) today"
        ];
    }
} catch (PDOException $e) {
    $notification_count = 0;
...
<head>
    ...
    <meta name="description" content="User Management - Speed of Light Airlines Admin" />
    <meta name="keywords" content="airline admin, user management, roles, customers, staff">
    <meta name="author" content="Speed of Light Airlines" />
    <title>User Management | Speed of Light Airlines</title>
    ...
    <style>
        ...
        * {
        ...
        /* FIXED: Ensure tables remain stable and visible */
        .table-container {
            position: relative;
            min-height: 200px;
        }
        ...
        /* Force table visibility - CRITICAL FIX */
        #staffTable, #customersTable {
            visibility: visible !important;
            opacity: 1 !important;
            display: table !important;
        }

        #staffBody, #customersBody {
            visibility: visible !important;
            opacity: 1 !important;
...
<body>
    ...
    <header class="topbar d-flex align-items-center">
        ...
        <div class="ticker ms-3 me-3 flex-grow-1 d-none d-md-block">
            <div class="ticker-track">
                <span>User Management • View customers & staff • Add / update / deactivate accounts • Assign roles • Welcome back <?php echo htmlspecialchars($admin_name); ?> • </span>
                <span>User Management • View customers & staff • Add / update / deactivate accounts • Assign roles • Welcome back <?php echo htmlspecialchars($admin_name); ?> • </span>
            </div>
        </div>
    ...
    <main class="content">
        <div class="page-title reveal mb-3">
            ...
            <div class="text-muted">View all registered users (passengers & staff), add/update/deactivate accounts, and assign roles.</div>
        </div>
            ...
            <form id="addUserForm" class="row g-3" novalidate>
                ...
                <div class="col-12 mt-2">
                    <button class="action-chip chip-green" type="submit" id="addUserBtn">
                        <i data-feather="check"></i> Add User
                    </button>
                </div>
            </form>
        </div>

        <!-- Grids: Customers and Staff/Admins -->
        <div class="row g-3">
            <!-- Customers -->
            ...
            <div class="col-12 col-xl-5 reveal">
                <div class="smart-card h-100">
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-2">
                        <div class="fw-bold">Staff & Sub Admins (<?php echo count($subAdmins); ?>)</div>
                        <div class="d-flex gap-2 align-items-center">
                            <input type="text" id="searchStaff" class="form-control form-control-sm" placeholder="Search staff">
                            <select id="statusStaff" class="form-select form-select-sm" style="width:auto;min-width:130px">
                                <option value="">All Status</option>
                                <option value="active">Active</option>
                                <option value="inactive">Inactive</option>
                            </select>
                    ...
                    <div class="small text-muted mt-2" id="staffCount"><?php echo count($subAdmins); ?> staff/admin(s)</div>
                </div>
            </div>
        </div>
    </main>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i data-feather="edit-2"></i> Edit User</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editUserForm" class="row g-3" novalidate>
                        <input type="hidden" id="editType"><!-- customer|staff -->
    ...
    <div class="modal fade" id="resetModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-2">Enter a new password for this user.</div>
                    <input type="hidden" id="resetType"><!-- customer|staff -->
    ...
    <script>
        ...
        // COMPLETELY REMOVED intersection observer to prevent table disappearing
        // Instead, immediately show all reveal elements
        ...
        // Load data from PHP - FIXED: Ensure data is properly loaded
        let customers = <?php echo json_encode($customers); ?>;
        let subAdmins = <?php echo json_encode($subAdmins); ?>;

        console.log('Customers loaded:', customers.length);
        console.log('Sub Admins loaded:', subAdmins.length);
        ...
        function renderStaff() {
            ...
            const sf = statusStaff.value;
            
            // FIXED: Consistent status filtering like customers
            const filtered = subAdmins.filter(u => {
                if (q && !(u.full_name + u.email).toLowerCase().includes(q)) return false;
                if (sf && u.status !== sf) return false; // FIXED: Now consistent with customers
                return true;
            });
            staffCount.textContent = `${filtered.length} staff/admin(s)`;
            
            if (!staffBody) {
                console.error('staffBody not found');
                return;
            }

            // CRITICAL: Force visibility
            staffBody.style.display = 'table-row-group';
            staffBody.style.visibility = 'visible';
                ...
                staffBody.innerHTML = `
                    <tr>
                        <td colspan="5" class="text-center text-muted py-4">
                            <i data-feather="search" style="width:48px;height:48px;margin-bottom:12px;"></i><br>
                            ${q || sf ? 'No staff match your search.' : 'No sub admins found.'}
                        </td>
                    </tr>
                `;
                feather.replace();
            ...
            filtered.forEach((u) => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${formatDate(u.created_at)}</td>
                    <td>${statusBadge(u.status)}</td>
                    <td>${actions('staff', u.sub_admin_id, u)}</td>`;
        ...
        function renderAll() {
            ...
            // CRITICAL: Force table visibility after render
            setTimeout(() => {
                const staffTable = document.getElementById('staffTable');
                const staffBodyEl = document.getElementById('staffBody');
                if (staffTable) {
                    staffTable.style.display = 'table';
                    staffTable.style.visibility = 'visible';
                    staffTable.style.opacity = '1';
                }
                if (staffBodyEl) {
                    staffBodyEl.style.display = 'table-row-group';
                    staffBodyEl.style.visibility = 'visible';
                    staffBodyEl.style.opacity = '1';
                }
            }, 100);
        }

        // CRITICAL: Ensure tables are visible on load
        document.addEventListener('DOMContentLoaded', function() {
            console.log('DOM loaded, forcing table visibility...');
            
            // Force table visibility immediately
            const tables = ['customersTable', 'staffTable'];
            const bodies = ['customersBody', 'staffBody'];
        ...
        if (searchStaff) searchStaff.addEventListener('input', renderStaff);
        if (statusStaff) statusStaff.addEventListener('input', renderStaff);
        ...
        addUserForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                formData.append('role', document.getElementById('uRole').value);
                ...
                notify('Error adding user. Please try again.', 'error');
        ...
        // Table actions - FIXED: Single event handler for both tables
        function handleActionClick(e) {
            const btn = e.target.closest('button[data-action]');
            if (!btn) return;

            const action = btn.getAttribute('data-action');
            const type = btn.getAttribute('data-type'); // customer|staff
        ...
        // Add event listeners to both table bodies
        if (customersBody) customersBody.addEventListener('click', handleActionClick);
        if (staffBody) staffBody.addEventListener('click', handleActionClick);
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (type === 'customer') {
                        ...
                        const admin = subAdmins.find(a => a.sub_admin_id == id);
                        if (admin) admin.status = result.new_status;
                    }
                    renderAll();
                } else {
                    notify(result.message, 'error');
        ...
        function openEdit(type, id) {
            let user;
            if (type === 'customer') {
                user = customers.find(c => c.user_id == id);
            } else {
                user = subAdmins.find(a => a.sub_admin_id == id);
            }

            if (!user) {
                console.error('User not found:', type, id);
                return;
            }
        ...
        // CRITICAL: Periodic check to ensure tables remain visible
        setInterval(() => {
            const staffTable = document.getElementById('staffTable');
            const staffBodyEl = document.getElementById('staffBody');
            if (staffTable && staffTable.style.display === 'none') {
                console.log('Staff table hidden, forcing visibility...');
                staffTable.style.display = 'table';
                staffTable.style.visibility = 'visible';
                staffTable.style.opacity = '1';
            }
            if (staffBodyEl && staffBodyEl.style.display === 'none') {
                console.log('Staff body hidden, forcing visibility...');
                staffBodyEl.style.display = 'table-row-group';
                staffBodyEl.style.visibility = 'visible';
                staffBodyEl.style.opacity = '1';
            }
